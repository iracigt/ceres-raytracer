...
inline 
std::pair<std::vector<bvh::Triangle<Scalar> >, std::vector<std::array<bvh::Vector3<Scalar>, 3> > >
load_from_stream(std::istream& is) {
    ...
    for (auto [x, y, z] : tri_idx) {
        tri_norms.emplace_back(std::array{normals[x], normals[y], normals[z]});
    }

    return std::pair(triangles, tri_norms);
}
...
load_from_file(const std::string& file) {
    ...
    return std::pair(std::vector<bvh::Triangle<Scalar> >(), std::vector<std::array<bvh::Vector3<Scalar>, 3> >()) ;
}
